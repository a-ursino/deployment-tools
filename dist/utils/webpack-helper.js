...
function getWebpackConfig(config, dev = false) {
	...
	const filePath = _path2.default.join(process.cwd(), `${ config.get('srcJsPath') }${ file }`);
	...
	// output path
	webpackConfig.output.path = _path2.default.join(process.cwd(), config.get('buildPathJs'));
	...
	if (dev) {
		...
		webpackConfig.output.publicPath = `${ config.get('domain') }/${ config.get('projectName') }${ config.get('buildPathJs') }`;
	}
	...
	webpackConfig.entry.main = _path2.default.join(process.cwd(), `${ config.get('srcJsPath') }${ config.get('mainJs') }`);
	...
	if (config.get('mainBackoffileJs') !== undefined) {
		webpackConfig.entry['main-backoffice'] = _path2.default.join(process.cwd(), `${ config.get('srcJsPath') }${ config.get('mainBackoffileJs') }`);
		debug(`webpackConfig.entry.main-backoffice ${ webpackConfig.entry['main-backoffice'] }`);
	}
	...
	if (config.get('vendorsBackoffileJs') !== undefined) {
		webpackConfig.entry['vendors-backoffice'] = _path2.default.join(process.cwd(), `${ config.get('srcJsPath') }${ config.get('vendorsBackoffileJs') }`);
